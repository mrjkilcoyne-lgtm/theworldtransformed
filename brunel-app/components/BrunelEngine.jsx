...
// The Brunel Engine - Transform frustrations into actionable insight
export default function BrunelEngine() {
  const [stage, setStage] = useState('welcome'); // welcome, interview, generating, report, followup
  const [messages, setMessages] = useState([]);
  const [input, setInput] = useState('');
  const [problemSummary, setProblemSummary] = useState('');
  const [seriousnessConfirmed, setSeriousnessConfirmed] = useState(false);
  const [report, setReport] = useState(null);
  const [selectedTier, setSelectedTier] = useState('free'); // free, standard, council
  const [email, setEmail] = useState('');
  const [followUpRegistered, setFollowUpRegistered] = useState(false);
  
  // Interview questions - Anthropic style, structured
  const interviewFlow = [
    {
      id: 'initial',
  ...
  const handleAnswer = () => {
    ...
    // Check if this was the seriousness question
  ...
  const handleGenerateReport = () => {
    ...
    setTimeout(() => {
      const generatedReport = generateReport(answers, selectedTier, seriousnessConfirmed);
      setReport(generatedReport);
      setStage('report');
    }, selectedTier === 'council' ? 4000 : 2000);
  };

  const generateReport = (answers, tier, serious) => {
    // In production, this calls the API
    // For prototype, generate structured output
  ...
  const handleFollowUp = () => {
    if (!email) return;
    // In production, this registers for 1mo + 1yr follow-up emails
      ...
      <div className="min-h-screen bg-stone-50 py-8 px-4">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="text-center py-8 border-t border-stone-200">
            ...
            <button className="text-stone-800 underline hover:no-underline">
              Donate later →
            </button>
          </div>
        </div>
